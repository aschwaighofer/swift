...
protected:
  ...
  void visitReturnInst(ReturnInst *Inst) {
    ...
    if (clonedResult->getType().getASTType() !=
      ...
      if (clonedResult->getType().is<SILFunctionType>()) {
        clonedResult = getBuilder().createConvertFunction(
            RegularLocation::getAutoGeneratedLocation(), clonedResult,
            origResult->getType(),
            /*withoutActuallyEscaping*/ false);
      } else if (clonedResult->getType().isTrivial(
                     getBuilder().getFunction())) {
        clonedResult = getBuilder().createUncheckedTrivialBitCast(
            RegularLocation::getAutoGeneratedLocation(), clonedResult,
            origResult->getType());
      } else {
    ...
    for (auto idx : indices(Values)) {
      if (OrigValues[idx]->getType().getASTType() !=
        ...
        if (!Values[idx]->getType().isAddress()) {
          if (Values[idx]->getType().is<SILFunctionType>()) {
            Values[idx] = getBuilder().createConvertFunction(
                RegularLocation::getAutoGeneratedLocation(), Values[idx],
                OrigValues[idx]->getType(),
                /*withoutActuallyEscaping*/ false);
          } else if (Values[idx]->getType().isTrivial(
                         getBuilder().getFunction())) {
            Values[idx] = getBuilder().createUncheckedTrivialBitCast(
                RegularLocation::getAutoGeneratedLocation(), Values[idx],
                OrigValues[idx]->getType());
          } else {
  ...
  void visitStoreInst(StoreInst *Inst) {
    ...
    if (destType.getASTType() != srcType.getASTType()) {
      if (srcType.getASTType()->hasOpaqueArchetype()) {
        ...
        if (srcType.isTrivial(getBuilder().getFunction())) {
          src = getBuilder().createUncheckedTrivialBitCast(

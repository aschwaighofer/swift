...
void SILGenFunction::emitCurryThunk(SILDeclRef thunk) {
  ...
  auto thunkInfo = SGM.Types.getConstantInfo(TypeExpansionContext(F), thunk);
  ...
  if (resultTy != toClosure.getType()) {
    ...
    if (resultFnTy->isABICompatibleWith(closureFnTy).isCompatible()) {
      ...
      auto appliedFnPattern =
          SGM.Types.getConstantInfo(TypeExpansionContext(F), calleeRef)
...
SILGenFunction::emitGlobalFunctionRef(SILLocation loc, SILDeclRef constant,
  ...
  assert(constantInfo == getConstantInfo(TypeExpansionContext(F), constant));

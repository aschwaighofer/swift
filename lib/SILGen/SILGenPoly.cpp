...
static ManagedValue createThunk(SILGenFunction &SGF,
  ...
  if (expectedType->isNoEscape()) {
    return SGF.emitManagedRValueWithPartialApplyStackCleanup(thunkValue,
                                                             expectedTL);
  }
...
enum EscapeTransformDirection {
  ToNoEscape,
  FromNoEscape,
};

static CanSILFunctionType buildTransformEscapingThunkType(
    SILGenFunction &SGF, CanSILFunctionType &noEscapingType,
    CanSILFunctionType &escapingType, GenericEnvironment *&genericEnv,
    SubstitutionMap &contextSubs,
    EscapeTransformDirection direction) {
  ...
  auto convertFromType =
      direction == FromNoEscape ? noEscapingType : escapingType;
  ...
  CanGenericSignature genericSig;
  if (convertFromType->hasArchetype()) {
  ...
  auto contextConvention = ParameterConvention::Direct_Guaranteed;
  SmallVector<SILParameterInfo, 4> params;
  params.append(convertFromType->getParameters().begin(),
                convertFromType->getParameters().end());
  params.push_back(
      {convertFromType, convertFromType->getExtInfo().hasContext()
                            ? contextConvention
                            : ParameterConvention::Direct_Unowned});
  ...
  SmallVector<SILYieldInfo, 4> interfaceYields;
  for (auto &yield : convertFromType->getYields()) {
    auto yieldIfaceTy = yield.getType()->mapTypeOutOfContext();
    auto interfaceYield =
      yield.getWithType(yieldIfaceTy->getCanonicalType(genericSig));
    interfaceYields.push_back(interfaceYield);
  ...
  SmallVector<SILResultInfo, 4> interfaceResults;
  for (auto &result : convertFromType->getResults()) {
    auto resultIfaceTy = result.getType()->mapTypeOutOfContext();
    auto interfaceResult =
      result.getWithType(resultIfaceTy->getCanonicalType(genericSig));
    interfaceResults.push_back(interfaceResult);
  ...
  Optional<SILResultInfo> interfaceErrorResult;
  if (convertFromType->hasErrorResult()) {
    auto errorResult = convertFromType->getErrorResult();
    auto errorIfaceTy = errorResult.getType()->mapTypeOutOfContext();
    interfaceErrorResult = SILResultInfo(
        errorIfaceTy->getCanonicalType(genericSig),
        convertFromType->getErrorResult().getConvention());
  }
...
static void buildTransformEscapingThunkBody(SILGenFunction &SGF) {
  PrettyStackTraceSILFunction stackTrace(
      "emitting transform @noescape thunk in", &SGF.F);
...
ManagedValue SILGenFunction::createWithoutActuallyEscapingClosure(
  ...
  auto thunkType =
      buildTransformEscapingThunkType(*this, noEscapingFnTy, escapingFnTy,
                                      genericEnv, contextSubs, FromNoEscape);
  ...
  if (thunk->empty()) {
    ...
    buildTransformEscapingThunkBody(thunkSGF);
  }
...
ManagedValue
SILGenFunction::createNoEscapingClosure(SILLocation loc,
                                        ManagedValue escapingFunctionValue,
                                        SILType noEscapingTy) {

  auto noEscapingFnTy = noEscapingTy.castTo<SILFunctionType>();
  auto escapingFnTy = escapingFunctionValue.getType().castTo<SILFunctionType>();

  assert(noEscapingFnTy->getExtInfo() ==
         escapingFnTy->getExtInfo().withNoEscape(true));

  GenericEnvironment *genericEnv = nullptr;
  SubstitutionMap contextSubs;
  auto thunkType = buildTransformEscapingThunkType(
      *this, noEscapingFnTy, escapingFnTy, genericEnv, contextSubs, ToNoEscape);

  auto *thunk = SGM.getOrCreateReabstractionThunk(
      thunkType, escapingFnTy, noEscapingFnTy, F.isSerialized());
  ...
  return emitManagedRValueWithPartialApplyStackCleanup(thunkedFn);
}
...
ManagedValue Transform::transformFunction(ManagedValue fn,
  ...
  assert(!(fnType->isNoEscape() && !expectedFnType->isNoEscape()) &&
         "Unexpected conversion from noescape to escape");

  // FIXME: Cleanup: I think the change to @noescape could just be handled by
  // createThunk above. createThunk already handles the conversion. And there is
  // really an ABI difference.

  auto changeToNoEscape = !fnType->isNoEscape() && expectedFnType->isNoEscape();

  // We do not, conversion is trivial.
  auto expectedEI = changeToNoEscape
                        ? expectedFnType->getExtInfo().withNoEscape(false)
                        : expectedFnType->getExtInfo();
  ...
  if (changeToNoEscape) {
    auto noEscapeFnType =
        adjustFunctionType(newFnType, newFnType->getExtInfo().withNoEscape(),
                           expectedFnType->getCalleeConvention(),
                           newFnType->getWitnessMethodConformanceOrNone());
    fn = SGF.createNoEscapingClosure(
        Loc, fn, SILType::getPrimitiveObjectType(noEscapeFnType));
    assert(fn.getType().castTo<SILFunctionType>() == expectedFnType);

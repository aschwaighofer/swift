...
static CanSILFunctionType getSILFunctionType(
    ...
    AnyFunctionType::ExtInfo extInfo, const Conventions &conventions,
...
static CanSILFunctionType getNativeSILFunctionType(
    ...
    AnyFunctionType::ExtInfo extInfo, Optional<SILDeclRef> origConstant,
    Optional<SILDeclRef> constant, Optional<SubstitutionMap> reqtSubs,
    Optional<ProtocolConformanceRef> witnessMethodConformance) {
...
getSILFunctionTypeForObjCSelectorFamily(TypeConverter &TC, ObjCSelectorFamily family,
  ...
  return getSILFunctionType(TC, TypeExpansionContext::minimal(),
                            AbstractionPattern(origType), substInterfaceType,
...
public:
  ...
  CanSILFunctionType substSILFunctionType(CanSILFunctionType origType) {
    ...
    return SILFunctionType::get(
        shouldSubstituteOpaqueArchetypes ? origType->getGenericSignature()
                                         : nullptr,
        extInfo, origType->getCoroutineKind(), origType->getCalleeConvention(),
        substParams, substYields, substResults, substErrorResult, TC.Context,
        witnessMethodConformance);
  }
...
SILFunctionType::substGenericArgs(SILModule &silModule, SubstitutionMap subs,
  ...
  return substGenericArgs(silModule, QuerySubstitutionMap{subs},
                          LookUpConformanceInSubstitutionMap(subs), context);
}

CanSILFunctionType
SILFunctionType::substGenericArgs(SILModule &silModule, TypeSubstitutionFn subs,
                                  LookupConformanceFn conformances,
                                  TypeExpansionContext context) {
...
SILFunctionType::substituteOpaqueArchetypes(TypeConverter &TC,
  ...
  assert(!(getGenericSignature() && TC.getCurGenericContext()));
...
TypeExpansionContext::TypeExpansionContext(const SILFunction &f)
    : expansion(f.getResilienceExpansion()),
      inContext(f.getDeclContext() ? f.getDeclContext()

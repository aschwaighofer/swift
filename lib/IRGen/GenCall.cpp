...
NativeConventionSchema::NativeConventionSchema(IRGenModule &IGM, SILType Type,
                                               bool IsResult)
    : Lowering(IGM.ClangCodeGen->CGM()) {
  if (auto *ti = dyn_cast<LoadableTypeInfo>(&IGM.getTypeInfo(Type))) {
    // Lower the type according to the Swift ABI.
    ti->addToAggLowering(IGM, Lowering, Size(0));
    Lowering.finish();
...
llvm::Type *SignatureExpansion::expandDirectResult() {
  ...
  case SILFunctionLanguage::Swift: {
    NativeConventionSchema schema(IGM, resultType, true);
    ...
    return schema.getExpandedType(IGM);
  }
  }
...
void SignatureExpansion::expand(SILParameterInfo param) {
  ...
  case ParameterConvention::Direct_Deallocating:
    ...
    case SILFunctionLanguage::Swift: {
      NativeConventionSchema nativeSchema(IGM, param.getSILType(), false);
...
void CallEmission::emitToUnmappedExplosion(Explosion &out) {
  ...
  NativeConventionSchema nativeSchema(IGF.IGM, resultType, true);
  ...
  if (result->getType() != expectedNativeResultType) {
    ...
    assert(origFunctionType->getLanguage() == SILFunctionLanguage::C);
    result =
...
bool irgen::addNativeArgument(IRGenFunction &IGF, Explosion &in,
  ...
  auto paramType = origParamInfo.getSILType();
  auto &ti = cast<LoadableTypeInfo>(IGF.getTypeInfo(paramType));
  auto schema = ti.getSchema();
  NativeConventionSchema nativeSchema(IGF.IGM, paramType, false);
  if (nativeSchema.requiresIndirect()) {
...
void IRGenFunction::emitScalarReturn(SILType resultType, Explosion &result,
  ...
  if (result.size() == 0) {
    assert(NativeConventionSchema(IGM, resultType, true).empty() &&
  ...
  if (isSwiftCCReturn) {
    NativeConventionSchema nativeSchema(IGM, resultType, true);
    assert(!nativeSchema.requiresIndirect());

    Explosion native = nativeSchema.mapIntoNative(IGM, *this, result, resultType);
    if (native.size() == 1) {

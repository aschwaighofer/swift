...
/// Emit a reference to the witness for a foreign type.
...
void WitnessTableBuilder::buildAccessFunction(llvm::Constant *wtable) {
  ...
  // We will unique the witness table if it is for a foreign type.
  auto uniqueWitnessIfIsForeignType =
  ...
  if (!RequiresSpecialization) {
    auto res = uniqueWitnessIfIsForeignType(wtable);
  ...
  // Possibly unique the witness if this is a foreign type witness.
  IGF.Builder.CreateRet(uniqueWitnessIfIsForeignType(call));
}

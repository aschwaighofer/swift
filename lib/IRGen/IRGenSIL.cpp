...
void IRGenSILFunction::emitSILFunction() {
  ...
  if (funcTy->isAsync() && funcTy->getLanguage() == SILFunctionLanguage::Swift)
    emitAsyncFunctionPointer(IGM, CurSILFn, CurFn,
...
void IRGenSILFunction::visitPartialApplyInst(swift::PartialApplyInst *i) {
  ...
  if (i->getOrigCalleeType()->isAsync()) {
    ...
    auto size =
        getDynamicAsyncContextSize(*this, layout, i->getOrigCalleeType(),
                                   std::get<0>(result), innerContext);
    llArgs.add(size);
  }
...
static void emitReturnInst(IRGenSILFunction &IGF,
  ...
  if (IGF.IndirectReturn.isValid()) {
    ...
    // TODO: call swift_asyncReturn or inline it's semantics.

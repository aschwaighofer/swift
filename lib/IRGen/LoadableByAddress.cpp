...
static llvm::cl::opt<bool> EnablePeepholes(
    "enable-large-value-peepholes", llvm::cl::init(true),
    llvm::cl::desc("enable large loadable types pass peepholes"));
...
static bool isLargeLoadableType(GenericEnvironment *GenericEnv, SILType t,
                                irgen::IRGenModule &Mod, bool includeTuples = false) {
  ...
  if (canType.getAnyGeneric() || (includeTuples && isa<TupleType>(canType))) {
...
static void runPeepholes(SILPassManager *pm, SILModule *silMod,
                         IRGenModule *irgenModule);
...
void LoadableByAddress::run() {  
  ...
  runPeepholes(getPassManager(), getModule(), getIRGenModule());
}
...
struct Peepholes {
  ...
  Peepholes(SILPassManager *pm, SILModule *silMod,
            IRGenModule *irgenModule)
...
bool Peepholes::optimizeLoad(SILBasicBlock &BB, SILInstruction *I) {
  ...
  // ->
  // copy_addr %3 to [init] %0uto nextIt = std::next(SILBasicBlock::iterator(LI));
  ...
  if (auto *copyAddr = dyn_cast<CopyAddrInst>(&*nextIt)) {
    if (copyAddr->getDest() != LI->getOperand() ||
...
public:
  ...
  void finish(DominanceInfo *dominance,
...
protected:
  ...
  void visitApplyInst(ApplyInst *apply) {
    ...
    for (auto &opd : apply->getAllOperands()) {
      if (assignment.contains(opd.get())) {
        auto builder = assignment.getBuilder(apply->getIterator());
        auto loaded = builder.createLoad(apply->getLoc(),
                                         assignment.getAddressForValue(opd.get()),
                                         LoadOwnershipQualifier::Unqualified);
        opd.set(loaded);
      }
    }
  ...
  void visitStructInst(StructInst *s) {
    ...
    for (auto &opd : s->getAllOperands()) {
      auto projAddr =
        builder.createStructElementAddr(s->getLoc(), structAddr,
                                        *fieldIt,
  ...
  void visitTupleInst(TupleInst *t) {
    ...
    for (auto &opd : t->getAllOperands()) {
      auto projAddr =
        builder.createTupleElementAddr(t->getLoc(), tupleAddr,
                                       opd.getOperandNumber(),
                                       opd.get()->getType().getAddressType());
      if (assignment.contains(opd.get())) {
        auto opdAddr = assignment.getAddressForValue(opd.get());
  ...
  void visitTupleExtractInst(TupleExtractInst *extract) {
    ...
    auto projAddr =
        builder.createTupleElementAddr(extract->getLoc(), opdAddr,
                                       extract->getFieldIndex(),
                                       extract->getType().getAddressType());
    builder.createCopyAddr(extract->getLoc(), projAddr, extractAddr, IsTake,
                           IsInitialization);
  ...
  void visitStructExtractInst(StructExtractInst *extract) {
    ...
    auto opdAddr = assignment.getAddressForValue(extract->getOperand());
    auto projAddr =
      builder.createStructElementAddr(extract->getLoc(),
                                      opdAddr,
                                      extract->getField(),
                                      extract->getType().getAddressType());
    builder.createCopyAddr(extract->getLoc(), projAddr, extractAddr, IsTake,
  ...
  void visitUncheckedTrivialBitCastInst(UncheckedTrivialBitCastInst *bc) {
    ...
    if (assignment.isLargeLoadableType(bc->getType()) &&
      ...
      auto opdAddr = builder.
        createUncheckedAddrCast(bc->getLoc(), addr,
                                bc->getOperand()->getType().getAddressType());
    ...
    auto newAddr = builder.createUncheckedAddrCast(bc->getLoc(),
                                                   opdAddr,
                                                   bc->getType().getAddressType());
    assignment.mapValueToAddress(origValue, newAddr);
...
protected:
  ...
  void visitEnumInst(EnumInst *e) {
    ...
    builder.createCopyAddr(e->getLoc(), opdAddr, initAddr, IsTake,
                           IsInitialization);
    builder.createInjectEnumAddr(e->getLoc(), enumAddr, e->getElement());
    auto enumVal = builder.createLoad(e->getLoc(),
                                      enumAddr,
                                      LoadOwnershipQualifier::Unqualified);
 ...
 void visitTryApplyInst(TryApplyInst *apply) {
    visitApplySite(apply);
  }

  void visitPartialApplyInst(PartialApplyInst *apply) {
    visitApplySite(apply);
  }
  ...
  void visitSelectEnumInst(SelectEnumInst *select) {
    ...
    auto res =
        builder.createSelectEnumAddr(select->getLoc(), opAddr,
                                     select->getType(),
                                     defaultValue,
                                     caseValues);
  ...
  void visitSwitchEnumInst(SwitchEnumInst *sw) {
    ...
    auto rewriteCase = [&](EnumElementDecl *caseDecl, SILBasicBlock *caseBB) {
      ...
      if (assignment.isLargeLoadableType(caseArg->getType())) {
        ...
        auto *caseLoad = caseBuilder.createLoad(loc, caseAddr,
                                                LoadOwnershipQualifier::Unqualified);
  ...
  void visitValueMetatypeInst(ValueMetatypeInst *metatype) {
    ...
    auto loaded = builder.createLoad(loc, opdAddr,
                                     LoadOwnershipQualifier::Unqualified);
    auto newVal = builder.createValueMetatype(loc, metatype->getType(),
                                              loaded);
...
static void runPeepholes(SILPassManager *pm, SILModule *silMod,
  ...
  for (SILFunction &currF : *silMod) {
    if (!currF.isDefinition())
      continue;
    auto *AA = pm->getAnalysis<AliasAnalysis>(&currF);
    ...
    // Assign addresses to basic block argument.
    SmallVector<std::pair<AllocStackInst*, SILValue>, 8> largeArguments;
    SmallVector<std::pair<AllocStackInst*, SILArgument*>, 8> largeTryApplyResults;

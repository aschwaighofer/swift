...
IRGenModule::getObjCProtocolRefSymRefDescriptor(ProtocolDecl *protocol) {
  ...
  auto typeRef = getTypeRef(ty, CanGenericSignature(), MangledTypeRefRole::FlatUnique).first;
  ...
  auto *protocolSymRef =
      new llvm::GlobalVariable(Module, future.getType(), /*constant*/ true,
                               llvm::GlobalValue::WeakAnyLinkage,
                               nullptr,
                               llvm::Twine("\01l_OBJC_PROTOCOL_SYMREF_$_") +
                               protocolName);
  future.installInGlobal(protocolSymRef);
  protocolSymRef->setAlignment(llvm::MaybeAlign(getPointerAlignment().getValue()));

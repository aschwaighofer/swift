...
static llvm::Value *unsafeContinuationFromTask(IRGenFunction &IGF,
  ...
  auto &nativeObjectTI = IGM.getNativeObjectTypeInfo();
  // Return UnsafeContinuation<T> at +1.
  auto object =
      Builder.CreateBitOrPointerCast(currTask, nativeObjectTI.getStorageType());
  IGF.emitStrongRetain(object, ReferenceCounting::Native,
                       IGF.getDefaultAtomicity());

  // Wrap the native object in the UnsafeContinuation struct.
  //   struct UnsafeContinuation<T> {
  //     let _continuation : Builtin.NativeObject
  //   }

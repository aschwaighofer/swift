...
namespace {
  ...
  public:
    ...
    void destroyStackFunction(IRGenFunction &IGF, Explosion &e,
                              SILType T) const {
      assert(T.getAs<SILFunctionType>()->isNoEscape() &&
             "Only @noescape functions support stack destruction");
      e.claimNext();
      auto context =
          IGF.Builder.CreateBitCast(e.claimNext(), IGF.IGM.RefCountedPtrTy);
      IGF.emitNativeStrongRelease(context, IGF.getDefaultAtomicity());
    }
  };
...
void irgen::destroyPartialApplyStack(IRGenFunction &IGF, Explosion &closure,
                                     SILType closureType) {
  auto &closureTI = IGF.getTypeInfo(closureType).as<FuncTypeInfo>();
  closureTI.destroyStackFunction(IGF, closure, closureType);
}

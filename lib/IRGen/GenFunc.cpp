...
static llvm::Function *emitPartialApplicationForwarder(IRGenModule &IGM,
...

  {
    ...
    NativeConventionSchema nativeSchema(IGM, outConv.getSILResultType(), true);
    ...
    for (unsigned i = 0; i < outType->getParameters().size(); ++i) {
      ...
      NativeConventionSchema nativeSchemaOrigParam(IGM, origParamSILType,
                                                   false);
      ...
      NativeConventionSchema nativeSchemaOutTypeParam(IGM, outTypeParamSILType,
                                                      false);

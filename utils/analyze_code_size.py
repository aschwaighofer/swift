...
class Categories(object):
    def __init__(self):
        self.category_matching = [
            ...
            ['Generic specialization of stdlib', 
                re.compile(r'.*generic specialization.* of (static )?(\(extension in Swift\):)?Swift\.')],
        ...
        self.specialization_matcher = re.compile(r'.*generic specialization <(?P<spec_list>[^>]*)>.* of (static )?(\(extension in Swift\):)?(?P<module_name>[^.]*)\.(?:(?P<first_type>[^.^(^<]*)\.){0,1}(?:(?P<last_type>[^.^(^<]*)\.)*(?P<function_name>[^(^<]*)')
        self.single_stdlib_specialized_type_matcher = re.compile(r'(Swift\.)?[^,^.]*$')
        self.two_specialized_stdlib_types_matcher = re.compile(r'(Swift\.)?[^,^.]*, (Swift\.)?[^,^.]*$')
        self.single_specialized_foundation_type_matcher = re.compile(r'(Foundation\.)?[^,^.]*$')
        self.two_specialized_foundation_types_matcher = re.compile(r'(Swift\.)?[^,^.]*, (Foundation\.)?[^,^.]*$')
        self.two_specialized_foundation_types_matcher2 = re.compile(r'(Foundation\.)?[^,^.]*, (Foundation\.)?[^,^.]*$')
        self.two_specialized_foundation_types_matcher3 = re.compile(r'(Foundation\.)?[^,^.]*, (Swift\.)?[^,^.]*$')
        self.array_type_matcher = re.compile(r'Array')
        self.dictionary = re.compile(r'Array')
        self.single_specialized_types_matcher = re.compile(r'(?P<module_name>[^,^.]*)\.(?P<type_name>[^,^.]*)$')
        self.is_class_type_dict = {}
        self.stdlib_and_other_type_matcher = re.compile(r'(Swift\.)?[^,^.]*, (?P<module_name>[^,^.]*)\.(?P<type_name>[^,^.]*)$')
        self.foundation_and_other_type_matcher = re.compile(r'(Foundation\.)?[^,^.]*, (?P<module_name>[^,^.]*)\.(?P<type_name>[^,^.]*)$')
    ...
    def group_library_types(self, module, type_name, specialization, mangled_name):
      ...
      if single_spec:
        ...
        is_dict = type_name != None and self.is_dictionary_like_type(type_name)
      ...
      foundation_other_spec = self.foundation_and_other_type_matcher.match(specialization)
    ...
    def add_specialization(self, symbol):
        ...
        if specialization_match:
          ...
          module, type_name, specialization = self.group_library_types(module, type_name, specialization, symbol.mangled_name)
          ...
          if existing_specialization:
            ...
            new_specialization = GenericSpecialization(module, type_name, specialization)
    ...
    def print_specializations(self):
       ...
       if not sorted_specializations:
         ...
         for spec in sorted_specializations:
           print("%20s.%s <%s> %8d" % (spec.module_name, spec.type_name, spec.specialization, spec.size))

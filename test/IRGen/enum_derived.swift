...
// CHECK-NORMAL-LABEL:define hidden swiftcc i1 @_TZFO12enum_derived1Eoi2eefTS0_S0__Sb(i8, i8)
// CHECK-TESTABLE-LABEL:define{{( protected)?}} swiftcc i1 @_TZFO12enum_derived1Eoi2eefTS0_S0__Sb(i8, i8)
// CHECK: %2 = icmp eq i8 %0, %1
// CHECK: ret i1 %2

// Check if the hashValue getter can be compiled to a simple zext instruction.

// CHECK-NORMAL-LABEL:define hidden swiftcc i{{.*}} @_TFO12enum_derived1Eg9hashValueSi(i8)
// CHECK-TESTABLE-LABEL:define{{( protected)?}} swiftcc i{{.*}} @_TFO12enum_derived1Eg9hashValueSi(i8)
...
// CHECK-NORMAL-LABEL: define hidden {{.*}}i64 @_TFO12enum_derived7Phantomg8rawValueVs5Int64(i8, %swift.type* nocapture readnone %T) local_unnamed_addr #1
// CHECK-TESTABLE-LABEL: define{{( protected)?}} {{.*}}i64 @_TFO12enum_derived7Phantomg8rawValueVs5Int64(i8, %swift.type* nocapture readnone %T) #1
